  ...
  return <Card className="w-full max-w-md mx-auto backdrop-blur-sm bg-card/95 shadow-card border-border/50">
      ...
      <CardContent className="space-y-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            <div className="relative">
              ...
              <Input {...register("name")} type="text" placeholder="Seu nome completo" className="pl-12" disabled={isLoading} />
            </div>
          ...
          <div className="space-y-2">
            <div className="relative">
              ...
              <Input {...register("email")} type="email" placeholder="seu@email.com" className="pl-12" disabled={isLoading} />
            </div>
          ...
          <div className="space-y-2">
            <div className="relative">
              ...
              <Input {...register("password")} type={showPassword ? "text" : "password"} placeholder="Crie uma senha forte" className="pl-12 pr-12" disabled={isLoading} />
          ...
          <div className="space-y-2">
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground w-5 h-5" />
              <Input {...register("confirmPassword")} type={showConfirmPassword ? "text" : "password"} placeholder="Confirme sua senha" className="pl-12 pr-12" disabled={isLoading} />
          ...
          <Button type="submit" disabled={isLoading} className="w-full bg-amber-100 text-zinc-600">
        ...
        <div className="grid grid-cols-1 gap-3">
          <Button variant="outline" onClick={() => handleSocialSignIn('google')} disabled={socialLoading !== null} className="w-full text-amber-50 border-white/20" style={{ backgroundColor: 'hsl(218, 15%, 32%)' }}>
            {socialLoading === 'google' ? <div className="w-5 h-5 mr-2 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
          ...
          <Button variant="outline" onClick={() => handleSocialSignIn('apple')} disabled={socialLoading !== null} className="w-full text-amber-50 border-white/20" style={{ backgroundColor: 'hsl(218, 15%, 32%)' }}>
            {socialLoading === 'apple' ? <div className="w-5 h-5 mr-2 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
